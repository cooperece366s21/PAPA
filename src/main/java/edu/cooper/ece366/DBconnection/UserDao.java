...
//    List<User> listUsers();
    ...
    @SqlUpdate("CREATE TABLE IF NOT EXISTS user (id VARCHAR(50) PRIMARY KEY, nickname VARCHAR(100)) ")
    void createTable();

    @SqlUpdate("INSERT INTO user(id, nickname) VALUES (?, ?)")
    void insertPositional(String id, String name);

    @SqlUpdate("INSERT INTO user(id, nickname) VALUES (:id, :name)")
    void insertNamed(@Bind("id") String id, @Bind("name") String name);

    @SqlUpdate("INSERT INTO user(id, nickname) VALUES (:ID, :nickname)")
    void insertBean(@BindBean User user);

    @SqlQuery("SELECT * FROM user ORDER BY nickname")
    @RegisterBeanMapper(User.class)
    List<User> listUsers();
